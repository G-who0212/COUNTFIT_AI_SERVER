...
class ChatConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data):
        ...
        ########################################## 운동 종료 신호 받은 것 추가함(9/26) ##############################################
        # 운동 종료 신호를 받는 경우
        if text_data_json['type'] == 'finish_exercise':
            # 비동기적으로 2초 대기한 후 최종 개수 전송
            async_to_sync(self.send_final_count)()
        ########################################## 운동 종료 신호 받은 것 추가함(9/26) ##############################################
...
########################################## 운동 종료 신호 받은 것 추가함(9/26) ##############################################
    async def send_final_count(self):
        print("운동 종료 신호 수신, 2초 대기 중...")
        await asyncio.sleep(0.5)  # 0.5초 대기

        # 0.5초 후 클라이언트에 최종 개수 전송
        self.send(text_data=json.dumps({
            'type': 'final_count',
            'final_count': self.count
        }))
        print(f"최종 운동 개수 {self.count} 클라이언트로 전송 완료.")
########################################## 운동 종료 신호 받은 것 추가함(9/26) ##############################################